...
const startPlanet = (planet) => {
    ...
    execSync(`npx wrangler d1 execute planet_db --file=schema.sql -c wrangler.dev.jsonc --local --persist-to=.wrangler/state/planet-${id}`, {
    ...
    const child = spawn('npx', [
        ...
        '--persist-to', `.wrangler/state/planet-${id}`,
        '--var', `PUBLIC_SIM_PLANET_NAME:"${name}"`,
        '--var', `PUBLIC_SIM_LANDING_SITE:"${url}"`,
        '--var', `PUBLIC_SIM_WARP_LINKS:'${env.PUBLIC_SIM_WARP_LINKS}'`
    ], {
        cwd: path.join(__dirname, '..'),
        env: process.env,
